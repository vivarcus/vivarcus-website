---
title: 里程碑跟踪与注册指标
description: 了解 Study Management 的里程碑跟踪（Milestone Tracking）与注册指标（Enrollment Metrics）：从 Plan Study 生成到排期完成、指标播种与 Metrics Over Time 的门禁。
...
**Study Management → Milestone Tracking** 列出全库研究里程碑（对象 Milestone，与 eTMF 共用），研究经理在此跟踪各研究关键节点：

| 列表列 | 含义 |
|--------|------|
| **Milestone** | 里程碑名（来自模板） |
| **Study / Study Country / Study Site** | 所属层级 |
| **Milestone Type** | 类型，如 `First Subject In (FSI)`、`Database Lock` |
| **Baseline / Planned / Actual Finish Date** | 基线、计划与实际完成日期 |
| **% Complete / Completeness** | 完成度（自动计算，图标呈现） |
| **Completed EDL Item Records** | 已完成的预期文档条目数 |

里程碑由 **Plan Study** 按主集模板自动生成（初始 **Unplanned**），国家/中心进入 Initiating 时生成各自层级里程碑。排期与完成：

1. **Plan Milestone**：填 **Planned Finish Date**，系统联动下游依赖日期，状态 → **Planned**。
2. **Mark Complete**：填 **Actual Finish Date**，状态 → **Complete**。

> 依赖门禁服务端强制：下游 **Mark Complete** 时上游未完成会被拒绝，除非设置 **Gating Override Date**。

完成度由「前序里程碑完成 + 必做任务完成 + 预期文档收集与批准」自动计算；用 **View Expected Documents** 查看关联 EDL Item。详细状态流转见 [里程碑](../etmf/milestones.html)。

## 注册指标

**Study Management → Enrollment Metrics** 维护各层级的入组指标（对象 Metric）。研究进入 Planning/Active、国家/中心进入 Initiating 时，系统**自动播种**每条指标一条记录（初值 0），按层级分三类：
...
执行对象工作流（研究/国家/中心详情页的 **All Actions**，如 Create Metrics Over Time）可按 **Recruitment Planning Frequency**（默认按月）生成各期间 planned/actual/forecast 记录。前置门禁：

- 研究的 **Metric Calculation** 为 Date-Based；
- 招募指标成对里程碑（First/Last）已有结束日期（Last ≥ First）；
- 对应指标的 **Planned > 0**。

生成的趋势数据叠加到 **Study Management Homepage** 的 Enrollment Status 折线图（仅研究/国家级展示，中心级入组看受试者列表）。

## 常见问题

| 现象 | 处理建议 |
|------|----------|
| Milestone Tracking 列表为空 | 研究须先执行 **Plan Study** 并推进到 Planning；国家/中心须进入 Initiating |
| Metrics Over Time 生成失败 | 核对三个门禁：Date-Based、First/Last 里程碑日期、Planned > 0 |
