---
title: 问题与方案偏离
description: 了解 Issue 的四种类型（Protocol Deviation / Observation / Protocol Violation / Risk Mitigation Action）、创建与推进方式、Start PD Review 的团队前置条件，以及与 eTMF 质量问题的对照。
...
| 类型 | 起始状态 | 常用动作 |
|------|----------|----------|
| **Protocol Deviation**（试验方案偏离） | **Open** | **Start PD Review** |
| **Observation**（观察） | **Open** | **Resolve Issue** 或 **Promote to Protocol Deviation** |
| **Protocol Violation**（试验方案违背） | **Open** | 按页面可用动作推进 |
| **Risk Mitigation Action**（风险缓解措施） | **Open** | **Assign Risk Mitigation** |

典型路径：**登记偏离（Open）→ 审查或解决 → Resolved**。

## 创建方案偏离

1. 导航 **Issue Management → Protocol Deviations**（或 **All Issues**）→ **Create**，类型选 **Protocol Deviation**。
2. 填写：
...
| **Study** | 必填 |
| **Study Country / Study Site** | 建议填写，便于仪表盘裁剪 |
| **Subject** | 关联受试者 |
...
3. 在 **My Tasks** 完成审查任务，完成后状态趋向 **Resolved**。
...
时间有限时的简化路径：**Issue Management → Observations** → Create，填写 Study / Site / Summary 保存（状态 Open），填写 **Resolution** 后 **All Actions → Resolve Issue**，状态变为 **Resolved**（Resolved Date 自动写入）。

> Protocol Deviation 与 Observation 可用动作不同：Observation 用 **Resolve Issue**；PD 优先 **Start PD Review**。Resolved 通常要求 Resolution 非空。

## 风险缓解措施（Risk Mitigation Action）

**Risk Mitigation Action（RMA）** 登记针对研究风险的缓解行动——「识别出风险后，决定做什么、由谁完成」。它与偏离/观察同属 Issue 对象、共用生命周期，但走专用的指派流程。**Issue Management → Risk Mitigation Actions** → Create：

| 字段 | 说明 |
|------|------|
| **Study** | 所属研究（必填，从研究风险评估带入） |
| **Study Country / Study Site** | 可选，按范围裁剪 |
...
**All Actions → Assign Risk Mitigation** 启动 **Complete Risk Mitigation** 工作流：
...
> 与其他类型一样，RMA 也可走通用动作（**Investigate / Implement / Escalate / Resolve Issue**）；**Change Issue Type** 可在 Issue 类型间转换（如 Observation 提升为 Risk Mitigation Action）。
...
| 对象动作 | Issue：**Start PD Review**、**Resolve Issue**、**Assign Risk Mitigation**、**Change Issue Type** | 审查、闭环与缓解措施指派 |
