...
| **已批准** | 已批准（版本约 1.0），进入稳态 |

## 提交审查

打开文档详情 → **所有操作 → 提交以供审查**：

1. 指定 **Reviewer** 与 **QC Reviewer**（可搜索 Vault 内用户；单账号试用时可选自己）。
...
| 任务 | 操作 | 结果 |
|------|------|------|
| **审查内容** | 在 **我的任务** 中打开并完成，选择 **发送至质检**（无需先接受） | 状态变为 In QC Review |
| **QC Review** | 完成，选择 **Passed** | 状态变为 In Approval |
| **批准文档** | 文档 **所有操作 → 批准**，指定 Approver 提交；在 **我的任务** 完成批准任务（含电子签名，若系统要求） | 状态变为 Approved，版本约 1.0 |

> 时间有限时可走快捷方式：直接 **所有操作 → 提升为已批准**，跳过审查步骤。
...
| 现象 | 处理建议 |
|------|----------|
| 找不到提交以供审查 | 确认文档已保存，且当前账号有文档操作权限 |
| 启动对话框选不到人 | 直接搜索姓名或邮箱；不必先在 Sharing 里授予 Reviewer / QC Reviewer |
...
| 生命周期动作 | Document：**提交以供审查**、**审查内容**、**QC Review**、**批准**、**提升为已批准** | 推进文档生命周期 |
