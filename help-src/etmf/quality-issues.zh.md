...
description: Quality Issue 记录文档缺陷并跟踪从发现到关闭的整改。了解四种状态、创建方式、与文档的双向关联，以及在 TMF Homepage 中的统计视图。
...
| 状态 | 含义 | 可执行动作 |
|------|------|------------|
| **Initiated** | 已记录，尚未分配/跟进 | **Open** |
| **Open** | 跟进中，等待整改 | **Close** |
| **Closed** | 已解决/已验证 | **Deactivate** |
| **Inactive** | 已归档（不在默认列表出现） | 只读 |

典型流转：**Initiated → Open → Closed**；需要从活跃视图移除时再 Deactivate。
...
1. 打开文档详情 → **Quality Issues** 相关对象区 → **Create**（从文档创建时 Related Document / Study 已预填）。
2. 填写：
...
**TMF Homepage → Quality Issues** Widget 按类型分组展示，可过滤 **Open / Closed / 全部 / 分配给我的**。创建并 Open 的 QI 会出现在 Open 视图中。
...
| 生命周期动作 | Quality Issue：**Open**、**Close**、**Deactivate** | 推进整改闭环 |
