...
1. 打开研究 → **预期文档** 列表，选择一条条目（如 Protocol 相关）打开详情，确认 **文档类型 / Classification**、**# Expected**、**Completeness** 等字段。
2. **所有操作 → 创建文档**。
...
| 现象 | 处理建议 |
|------|----------|
| 找不到创建文档 | 确认当前账号有预期文档操作权限；条目状态为 Active |
| Matched Documents 为空 | 确认文档的 Study、Classification 等与 EDL Item 一致；等待几秒刷新，或执行匹配文档 |
...
| 对象动作 | EDL Item：**创建文档** | 从预期文档条目创建（推荐路径） |
