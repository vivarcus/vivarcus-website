...
description: TMF Homepage 把完整性、及时性、质量问题与待办汇总成一张健康仪表盘。了解各 Widget 的口径、Study 选择器与稽查就绪自检清单。
...
TMF Homepage 是研究健康度的日常入口，可视化 TMF 文档的**及时性、完整性与质量**，并汇总任务与里程碑。顶部 **Study 选择器**：Study 必填，Study Country / Study Site 可选，用于把指标裁剪到国家或中心范围。
...
| **Quality Issues** | 质量问题饼图，可过滤 Open / Closed / 全部 |
| **My Tasks** | 最多 10 条分配给我的任务 |
| **Tasks Requiring Attention** | Overdue / Unassigned / Due Today 三计数 |
...
- **Quality Issues** 按类型（Duplicate / Expired / Misclassified 等）分组，可过滤 Open、Closed、全部或分配给我的。需要在相关文档类型上配置 Quality Issue 字段后才会统计。
- **My Tasks** 只显示 document、envelope 与 clinical user tasks；质量问题任务在 Quality Issues Widget 中查看。
...
| 类型 | 权限 | 作用 |
|------|------|------|
| Tab | TMF Homepage：View | 访问 TMF Homepage 标签 |
| 对象 | Study / Milestone / EDL Item：Read | Completeness、Upcoming Milestones 等 Widget 取数 |
| 对象 | Document：Read | Unapproved Documents 计数与下钻 |
| 对象 | Clinical User Task：Read | My Tasks、Tasks Requiring Attention |
| 对象 | Quality Issue：Read | Quality Issues Widget 统计 |
