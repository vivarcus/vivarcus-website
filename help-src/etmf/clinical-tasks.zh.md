...
**Clinical User Task**（临床用户任务）是挂在研究层级上的待办/问题条目：指派给某人，跟踪类别、优先级、截止日期，直至解决。任务出现在多个入口，eTMF 侧主要是 **Planning → Clinical Tasks**；CTMS 侧的监查跟进项也基于同一对象（见 [监查跟进项与 CTMS 临床任务](../ctms/clinical-tasks.html)）。
...
| 类型 | 层级 | 说明 |
|------|------|------|
| **Study Task**（默认） | 研究 | 研究级任务，可关联里程碑与模板任务 |
| **Study Country Task** | 研究国家/地区 | 国家/地区级任务 |
| **Study Site Task** | 研究地点 | 中心级任务 |
| **Monitoring Follow Up Item** | 研究中心 | CTMS 监查跟进项，必须关联同一中心的监查事件 |

研究/国家/中心详情页的 **Study Tasks / Study Country Tasks / Study Site Tasks** 区块按类型过滤展示对应任务。

## 创建任务

1. 导航 **Planning → Clinical Tasks** → 创建，选择类型（默认 **Study Task**）。
2. 填写：

| 字段 | 说明 |
|------|------|
| **Name** | 任务名称（必填） |
| **Study / Study Country / Study Site** | 按类型级联选择 |
...
3. 保存后任务处于 **Open** 状态，被指派人会收到通知；**My Tasks** 页可集中处理所有指派给自己的任务。

## 状态与动作

| 状态 | 可用动作 |
|------|----------|
| **Open** | **Investigate**（开始调查）、**Implement**（直接实施）、**Escalate**（上报）、**Resolve**（解决） |
| **Investigating** | **Implement**、**Escalate**、**Resolve** |
| **Implementing** | **Escalate**、**Resolve** |
| **Escalated** | **Investigating**、**Implementing**、**Resolve** |
| **Resolved** | 无（终态，记录变为只读） |
...
| 现象 | 处理建议 |
|------|----------|
| 创建时没有 Milestone 字段 | 使用默认类型 Study Task；Monitoring Follow Up Item 类型字段集更精简 |
| Resolved 后想修改 | Resolved 为终态只读；如需修正请重新创建任务 |
| 收不到任务通知 | 确认任务已填写 **Assigned To**；指派与改派通知均发给被指派人 |
